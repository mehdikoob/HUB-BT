...
async def export_incident_report(
    ...
    # Ajouter les pièces jointes et screenshots du test
    ...
    if test_attachments or test_screenshots:
        ...
        # Afficher les screenshots du test
        if test_screenshots:
            story.append(Paragraph("<b>Captures d'écran :</b>", normal_style))
            story.append(Spacer(1, 0.05*inch))
            
            screenshot_images = []
            for screenshot_id in test_screenshots[:3]:
                try:
        ...
        # Afficher les fichiers joints (attachments)
        if test_attachments:
            story.append(Paragraph("<b>Fichiers joints :</b>", normal_style))
            story.append(Spacer(1, 0.05*inch))
            
            # Récupérer et afficher les fichiers
            for attachment in test_attachments[:5]:  # Max 5 fichiers
                try:
                    # Les attachments sont stockés comme URLs dans UPLOAD_DIR
                    ...
                    # Vérifier si c'est une image (jpeg, png)
                    if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
                        # Essayer d'afficher l'image
                        file_path = UPLOAD_DIR / filename
                        if file_path.exists():
                            try:
                                img = Image(str(file_path), width=2.5*inch, height=2*inch)
                                story.append(img)
                                story.append(Paragraph(f"<i>{filename}</i>", normal_style))
                                story.append(Spacer(1, 0.1*inch))
                            except Exception as e:
                                # Si l'image ne peut pas être chargée, afficher juste le nom
                                story.append(Paragraph(f"📎 {filename}", normal_style))
                                story.append(Spacer(1, 0.05*inch))
                        else:
                            story.append(Paragraph(f"📎 {filename}", normal_style))
                            story.append(Spacer(1, 0.05*inch))
                    else:
                        # Pour les PDF et autres fichiers, afficher juste le nom
                        story.append(Paragraph(f"📎 {filename} (fichier joint)", normal_style))
                        story.append(Spacer(1, 0.05*inch))
...
async def get_insights(
    ...
    programme_id: str = Query(None),
    partenaire_id: str = Query(None),
    current_user: User = Depends(get_current_active_user)
):
    """
    Générer des insights intelligents avec Gemini AI
    ...
    insights = await generate_insights_with_ai(period, programme_id, partenaire_id)
    return insights
