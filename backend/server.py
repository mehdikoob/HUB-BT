...
async def create_test_site(input: TestSiteCreate, current_user: User = Depends(get_current_active_user)):
    ...
    existing_test = await db.tests_site.find_one({
    ...
    if existing_test:
        ...
        raise HTTPException(
            status_code=409,
            detail=f"Un test site existe déjà ce mois pour {partenaire.get('nom', 'ce partenaire')} / {programme.get('nom', 'ce programme')} (créé par {creator_name})"
        )
...
async def get_tests_site(
    ...
    if paginate:
        skip = (page - 1) * limit
        tests = await db.tests_site.find(query, {"_id": 0}).sort("date_test", -1).skip(skip).limit(limit).to_list(limit)
    else:
        # Sans pagination: limite à 5000 pour compatibilité
        tests = await db.tests_site.find(query, {"_id": 0}).sort("date_test", -1).to_list(5000)
